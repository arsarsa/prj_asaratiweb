...
const {
  ...
  general: { allowed_origin: ALLOWED_ORIGIN = "https://asarati.it" } = {},
} = config();
...
const checkToken = (token) => {
  const validTokens = ["TOKEN1", "TOKEN2"]; // Personalizza
...
export const updateDownloadCount = onCall(async (data, context) => {
  const { token, ebookId, email, downloadType, fileExtension = "pdf", isNewsletterSubscribed = false } = data;
  ...
  try {
    ...
    const emailHtml = `
      ...
      ${newsletterMsg}<br><br>
      Se hai problemi, contattaci a <a href="mailto:support@asarati.it">support@asarati.it</a>
    `;

    await sendEmailWithTimeout({
      to: email,
      from: SENDER_EMAIL,
      subject: "Il tuo download da asarati.it",
      text: emailText,
    });
